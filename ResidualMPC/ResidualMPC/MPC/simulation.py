...
class SwingController:
    ...
    def __init__(
        ...
        # # ====== debug 相关 ======
        # self.debug_flight = True
        # self.debug_phase = 0.0
        # self.debug_freq = 0.5
        # self.debug_step = 0  # 用来当时间索引
    ...
    def _swing_traj(self, p0, p1, phi):
        ...
        这里：
        - x 用线性插值 + 常速
        - z 用一个抛物线，通过 (0,z0), (1,z1), (0.5,z_mid)
        - 顶点（接近 0.5）处只有水平速度
        """
    ...
    def update_and_compute_tau(self, contact_L_now: bool, contact_R_now: bool):
        ...
        for leg in self.legs:
            ...
            if was_contact and (not is_contact):
                ...
                # 落脚点：简单地往前迈 step_length 这里到时候可以优化
            ...
            # comx = compute_com(self.env)
            # p0, v0 = get_foot_state(self.env, leg)
            # print(comx)
            # print(p0,leg)
            ...
            if self.active[leg]:
                ...
                if leg == "back":
                    print(leg,"current_pos",p_now,"desired_pos",p_des)
                    print(leg,"current_vel",v_now,"desired_vel",v_des)
                e_p = p_des - p_now
                e_v = v_des - v_now
                if leg == "back":
                    print(leg,"ep",e_p,"ev",e_v)
                # 任务空间 PD
    ...
    def plot_debug_tracking(self):
        """
        使用 self.debug_log 画两张图：
          图1：位置跟踪（x,z 的 p_now vs p_des）
          图2：速度跟踪（x,z 的 v_now vs v_des）
        只包含 debug 模式下（both_air）的采样
        """
        import numpy as np
        import matplotlib.pyplot as plt

        if len(self.debug_log["t"]) == 0:
            print("No debug data recorded. Make sure debug_flight=True and both_air happened.")
            return

        t = np.array(self.debug_log["t"])
        leg = np.array(self.debug_log["leg"])

        p_now_x = np.array(self.debug_log["p_now_x"])
        p_now_z = np.array(self.debug_log["p_now_z"])
        p_des_x = np.array(self.debug_log["p_des_x"])
        p_des_z = np.array(self.debug_log["p_des_z"])

        v_now_x = np.array(self.debug_log["v_now_x"])
        v_now_z = np.array(self.debug_log["v_now_z"])
        v_des_x = np.array(self.debug_log["v_des_x"])
        v_des_z = np.array(self.debug_log["v_des_z"])

        # 为了好看，分前腿 / 后腿画
        legs_unique = ["back", "front"]

        # ===== 图 1：位置跟踪 =====
        fig1, axs1 = plt.subplots(2, 1, sharex=True, figsize=(8, 6))
        for lg in legs_unique:
            mask = (leg == lg)
            axs1[0].plot(t[mask], p_now_x[mask], linestyle='-',  label=f"{lg} p_now_x")
            axs1[0].plot(t[mask], p_des_x[mask], linestyle='--', label=f"{lg} p_des_x")
            axs1[1].plot(t[mask], p_now_z[mask], linestyle='-',  label=f"{lg} p_now_z")
            axs1[1].plot(t[mask], p_des_z[mask], linestyle='--', label=f"{lg} p_des_z")

        axs1[0].set_ylabel("x position")
        axs1[1].set_ylabel("z position")
        axs1[1].set_xlabel("time (s)")
        axs1[0].set_title("Foot position tracking (p_now vs p_des)")
        axs1[0].legend()
        axs1[1].legend()
        fig1.tight_layout()

        # ===== 图 2：速度跟踪 =====
        fig2, axs2 = plt.subplots(2, 1, sharex=True, figsize=(8, 6))
        for lg in legs_unique:
            mask = (leg == lg)
            axs2[0].plot(t[mask], v_now_x[mask], linestyle='-',  label=f"{lg} v_now_x")
            axs2[0].plot(t[mask], v_des_x[mask], linestyle='--', label=f"{lg} v_des_x")
            axs2[1].plot(t[mask], v_now_z[mask], linestyle='-',  label=f"{lg} v_now_z")
            axs2[1].plot(t[mask], v_des_z[mask], linestyle='--', label=f"{lg} v_des_z")

        axs2[0].set_ylabel("x velocity")
        axs2[1].set_ylabel("z velocity")
        axs2[1].set_xlabel("time (s)")
        axs2[0].set_title("Foot velocity tracking (v_now vs v_des)")
        axs2[0].legend()
        axs2[1].legend()
        fig2.tight_layout()
