...
class SRB_MPC:
    ...
    def __init__(
        ...
        x_ref=np.array([None, 0.5, 0.0, 1.0, 0.0, 0.0], dtype=object),
        # 权重（与你原来相近；px=0 不罚）s
        Q=np.diag([ 0,  80,  5,  1e2,  40, 10 ]),
        Qf=None,                      # 若 None，使用 Q
        R=np.diag([5e-4, 1e-3, 5e-4, 1e-3]),
        comp_eps=1e-3,              # 互补罚项软化参数
        comp_rho=1e2,              # 互补罚项权重
        use_nominal_support=True,     # 输入项使用 (u - u_ref)^T R (u - u_ref)
        ...
        self.mu = float(mu)
        self.comp_eps = float(comp_eps)
        self.comp_rho = float(comp_rho)
    ...
    def _foot_z(self, x, side):
        """
        给定状态 x，返回某只脚的世界坐标 z_height.
        side: "L" 或 "R"
        """
        ...
        z_world = pz + ca.cos(th) * z_body - ca.sin(th) * x_body
        return z_world
    # ---------------- build opti ----------------
    def _build(self):
        N = self.N
        ...
        for k in range(N):
            ...
            opti.subject_to(-U[2, k] <=  self.mu * U[3, k])      # -FxR <=  mu*FzR
                # 1. 计算脚–地面间隙 φ(x) = z_foot - ground_z
            zL = self._foot_z(xk, "L")
            zR = self._foot_z(xk, "R")
            gapL = zL - self.ground_z   # φ_L(x_k)
            gapR = zR - self.ground_z   # φ_R(x_k)

            # 2. 几何约束：脚不能穿地
            # opti.subject_to(gapL >= 0)
            # opti.subject_to(gapR >= 0)
            # # 4. 互补性：φ * Fz ≈ 0
            # #   纯等式 φ*Fz = 0 对 IPOPT 很硬，通常会收敛很难
            # #   建议用：φ*Fz <= eps + 在 cost 里加惩罚
            # opti.subject_to(gapL * U[1, k] <= self.comp_eps)
            # opti.subject_to(gapR * U[3, k] <= self.comp_eps)

            # 再加到 cost 里做 soft penalty
            comp_termL = gapL * U[1, k]
            comp_termR = gapR * U[3, k]
            cost += self.comp_rho * (comp_termL**2 + comp_termR**2)
        # 终端代价
        xN = X[:, N]
        xN_err = xN - xref
        xN_err = ca.vertcat(
...
if __name__ == "__main__":
    mpc = SRB_MPC(
        dt=0.05, N=20, m=14.0, I=3.5, g=9.81, foot_x=0.62, mu=0.4,
    ...
    # 参考（vx=1m/s, 其余默认）
    mpc.set_reference(px=None, pz=0.55, th=0.0, vx=1.0, vz=0.0, w=0.0)
