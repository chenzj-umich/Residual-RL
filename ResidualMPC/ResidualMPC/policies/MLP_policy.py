...
from ResidualMPC.policies.base_policy import BasePolicy
...
class MLPPolicySL(BasePolicy, nn.Module, metaclass=abc.ABCMeta):
    ...
    def update(self, observations, actions):
        ...
        if isinstance(observations, np.ndarray):
            ...
            observations = observations.to(dtype=torch.float32,device=ptu.device)
        if isinstance(actions, np.ndarray):
            actions = ptu.from_numpy(actions.astype(np.float32))
        else:
            actions = actions.to(dtype=torch.float32,device=ptu.device)

        prediction_dist = self(observations)
        loss = -prediction_dist.log_prob(actions).mean()
