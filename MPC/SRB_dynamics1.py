...
class SRB_MPC:
    ...
    def _f_cont(self, x, u):
        ...
        rxL= -foot_x
        # rzL =  ground_z - pz
        rzL = ca.fmax(pz - foot_x * np.sin(th) - ground_z, 0)
        rxR = foot_x
        # rzR = ground_z - pz
        rzR = ca.fmax(pz + foot_x * np.sin(th) - ground_z, 0)
        dpx, dpz, dth = vx, vz, w
        dvx = (FxL + FxR) / m
        dvz = (FzL + FzR) / m - g
        print( "rzL:",rzL)
    ...
    def _build(self):
        ...
        for k in range(N):
            ...
            if self.use_nominal_support:
                u_ref = ca.DM([0.0, self.m * self.g * 0.5, 0.0, self.m * self.g *0.5])
                u_err = uk - u_ref
            ...
            opti.subject_to(-U[0, k] <=  self.mu * U[1, k])      # -FxL <=  mu*FzL
            # opti.subject_to((X[1, k] - self.ground_z) * U[1, k] >= 0)            #  FzL * px >= 0
            # 右脚
            opti.subject_to(U[3, k] >= 0)                        # FzR >= 0
            opti.subject_to( U[2, k] <=  self.mu * U[3, k])      #  FxR <=  mu*FzR
            opti.subject_to(-U[2, k] <=  self.mu * U[3, k])      # -FxR <=  mu*FzR
            # opti.subject_to((X[1, k] - self.ground_z) * U[3, k] >= 0)            #  FzR * px >= 0
...
if __name__ == "__main__":
    mpc = SRB_MPC(
        ...
        use_nominal_support=True,
        ipopt_p_opts={"print_time": 1},
        ipopt_s_opts={"print_level": 0, "max_iter": 500, "tol": 1e-6},
    )

    cfg = env_wrapper.HCEnvConfig(render=True, obs_as_state=False, seed=0)
    env = env_wrapper.HalfCheetahEnv(cfg)
    obs, _ = env.reset()
    x0 = get_single_rigidbody_state(env) # [px, pz, th, vx, vz, w]
    ...
    try:
        for t in range(T):
            ...
            FxL, FzL, FxR, FzR = u0[0], u0[1], u0[2], u0[3]
